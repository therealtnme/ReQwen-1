...
from transformers.utils.generic import accepts_precomputed_kwargs, merge_with_config_defaults
...
@strict
class ReQwen_1TextConfig(Qwen3NextConfig):
    r"""
    ...
    ```python
    >>> from transformers import ReQwen_1TextModel, ReQwen_1TextConfig

    >>> # Initializing a Qwen3.5 style configuration
    >>> configuration =  ReQwen_1TextConfig()

    >>> # Initializing a model from the Qwen3.5-9B style configuration
    >>> model = ReQwen_1TextModel(configuration)
...
@auto_docstring(checkpoint="Qwen/Qwen3.5-27B")
@strict
class ReQwen_1VisionConfig(Qwen3VLVisionConfig):
    r"""
...
@auto_docstring(checkpoint="Qwen/Qwen3.5-27B")
@strict
class ReQwen_1Config(Qwen3VLConfig):
    r"""
    Example:

    ```python
    >>> from transformers import ReQwen_1ForConditionalGeneration, ReQwen_1Config

    >>> # Initializing a Qwen3.5 style configuration
    >>> configuration = ReQwen_1Config()

    >>> # Initializing a model from the Qwen3.5-9B style configuration
    >>> model = ReQwen_1ForConditionalGeneration(configuration)
...
class ReQwen_1VisionRotaryEmbedding(Qwen3VLVisionRotaryEmbedding):
    pass


class ReQwen_1TextRotaryEmbedding(Qwen3VLTextRotaryEmbedding):
    def __init__(self, config: ReQwen_1TextConfig, device=None):
        super().__init__()
        self.mrope_section = config.rope_parameters.get("mrope_section", [11, 11, 10])

    def compute_default_rope_parameters(
        config: ReQwen_1TextConfig | None = None,
...
@use_kernel_forward_from_hub("ReQwen_1GatedDeltaNet")
class ReQwen_1GatedDeltaNet(Qwen3NextGatedDeltaNet):
    def __init__(self, config: ReQwen_1Config, layer_idx: int):
...
class ReQwen_1Attention(Qwen3NextAttention):
    pass


class ReQwen_1MLP(Qwen3NextMLP):
    def __init__(self, config: ReQwen_1Config, intermediate_size: int):
        super().__init__(config, intermediate_size)
        self.intermediate_size = intermediate_size


class ReQwen_1RMSNorm(Qwen3NextRMSNorm):
    pass
...
class ReQwen_1DecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: ReQwen_1TextConfig, layer_idx: int):
        ...
        if self.block_type == "linear_attention":
            self.linear_attn = ReQwen_1GatedDeltaNet(config, layer_idx)
        elif self.block_type == "full_attention":
            self.self_attn = ReQwen_1Attention(config, layer_idx)
        self.mlp = ReQwen_1MLP(config, config.intermediate_size)
        self.input_layernorm = ReQwen_1RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = ReQwen_1RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class ReQwen_1PreTrainedModel(Qwen3NextPreTrainedModel):
    config: ReQwen_1Config
    _no_split_modules = ["ReQwen_1DecoderLayer", "ReQwen_1VisionBlock"]
    _can_record_outputs = {
        "hidden_states": ReQwen_1DecoderLayer,
        "attentions": ReQwen_1Attention,
    }

    @torch.no_grad()
    def _init_weights(self, module):
        PreTrainedModel._init_weights(self, module)
        if isinstance(module, ReQwen_1GatedDeltaNet):
            init.ones_(module.dt_bias)
            init.copy_(module.A_log, torch.empty_like(module.A_log).uniform_(0, 16).log_())
        # We initialize with 0s to be 1 centered as the RMSNorm here does (1 + weight)
        elif isinstance(module, ReQwen_1RMSNorm):
            init.zeros_(module.weight)
        elif isinstance(module, ReQwen_1VisionRotaryEmbedding):
            inv_freq = 1.0 / (module.theta ** (torch.arange(0, module.dim, 2, dtype=torch.float) / module.dim))
            init.copy_(module.inv_freq, inv_freq)


class ReQwen_1VisionModel(Qwen3VLVisionModel):
    config: ReQwen_1VisionConfig
    _no_split_modules = ["ReQwen_1VisionBlock"]
...
class ReQwen_1ModelOutputWithPast(Qwen3VLModelOutputWithPast):
    pass


class ReQwen_1TextModel(Qwen3NextModel):
    config: ReQwen_1TextConfig

    def __init__(self, config: ReQwen_1TextConfig):
        super().__init__(config)
        self.rotary_emb = ReQwen_1TextRotaryEmbedding(config=config)
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        return ReQwen_1ModelOutputWithPast(
            last_hidden_state=hidden_states,
            past_key_values=past_key_values,
        )


class ReQwen_1Model(Qwen3VLModel):
    _no_split_modules = ["ReQwen_1DecoderLayer", "ReQwen_1VisionBlock"]
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
    ) -> tuple | ReQwen_1ModelOutputWithPast:
        r"""
...
class ReQwen_1ForCausalLM(Qwen3ForCausalLM):
    config: ReQwen_1TextConfig
    _keys_to_ignore_on_load_unexpected = [r"^mtp.*", r"^model.visual.*"]

    def __init__(self, config):
        super().__init__(config)
        self.model = ReQwen_1TextModel(config)


class ReQwen_1ForTokenClassification(GenericForTokenClassification, ReQwen_1PreTrainedModel):
    config: ReQwen_1Config


class ReQwen_1ForConditionalGeneration(Qwen3VLForConditionalGeneration):
    def get_video_features(self, **super_kwargs) -> tuple | BaseModelOutputWithPooling:
        return super().get_video_features(**super_kwargs)
...
class ReQwen_1TextForSequenceClassification(GenericForSequenceClassification, ReQwen_1PreTrainedModel):
    config: ReQwen_1TextConfig
    input_modalities = ("text",)


class ReQwen_1ForSequenceClassification(GenericForSequenceClassification, ReQwen_1PreTrainedModel):
    def forward(
        self,
        input_ids: torch.LongTensor = None,
        attention_mask: torch.Tensor | None = None,
...
__all__ = [
    "ReQwen_1Config",
    "ReQwen_1TextConfig",
    "ReQwen_1VisionConfig",
    "ReQwen_1VisionModel",
    "ReQwen_1TextModel",
    "ReQwen_1Model",
    "ReQwen_1ForCausalLM",
    "ReQwen_1TextForSequenceClassification",
    "ReQwen_1ForSequenceClassification",
    "ReQwen_1ForTokenClassification",
    "ReQwen_1ForConditionalGeneration",
    "ReQwen_1PreTrainedModel",
]
