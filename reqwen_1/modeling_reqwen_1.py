...
from .configuration_reqwen_1 import ReQwen_1Config, ReQwen_1TextConfig, ReQwen_1VisionConfig
...
class ReQwen_1VisionRotaryEmbedding(nn.Module):
...
class ReQwen_1TextRotaryEmbedding(nn.Module):
    inv_freq: torch.Tensor  # fix linting for `register_buffer`
    ...
    def forward(self, x, position_ids):
        # In contrast to other models, ReQwen_1 has different position ids for the grids
...
class ReQwen_1RMSNormGated(nn.Module):
...
class ReQwen_1GatedDeltaNet(nn.Module):
    def __init__(self, config: ReQwen_1Config, layer_idx: int):
        ...
        self.norm = (
            ReQwen_1RMSNormGated(self.head_v_dim, eps=self.layer_norm_epsilon)
...
class ReQwen_1Attention(nn.Module):
    ...
    def __init__(self, config: ReQwen_1Config, layer_idx: int):
        ...
        self.q_norm = ReQwen_1RMSNorm(self.head_dim, eps=config.rms_norm_eps)  # unlike olmo, only on the head dim!
        self.k_norm = ReQwen_1RMSNorm(self.head_dim, eps=config.rms_norm_eps)  # thus post q_norm does not need reshape
...
class ReQwen_1RMSNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        # Llama does x.to(float16) * w whilst ReQwen_1 is (x * w).to(float16)
...
class ReQwen_1PreTrainedModel(PreTrainedModel):
...
class ReQwen_1VisionMLP(nn.Module):
...
class ReQwen_1VisionPatchEmbed(nn.Module):
...
class ReQwen_1VisionPatchMerger(nn.Module):
    def __init__(self, config: ReQwen_1VisionConfig, use_postshuffle_norm=False) -> None:
        super().__init__()
...
class ReQwen_1VisionAttention(nn.Module):
    def __init__(self, config: ReQwen_1VisionConfig) -> None:
        super().__init__()
...
class ReQwen_1VisionBlock(GradientCheckpointingLayer):
    def __init__(self, config, attn_implementation: str = "sdpa") -> None:
        super().__init__()
        self.norm1 = nn.LayerNorm(config.hidden_size, eps=1e-6)
        self.norm2 = nn.LayerNorm(config.hidden_size, eps=1e-6)
        self.attn = ReQwen_1VisionAttention(config=config)
        self.mlp = ReQwen_1VisionMLP(config=config)
...
class ReQwen_1VisionModel(ReQwen_1PreTrainedModel):
    config: ReQwen_1VisionConfig
    input_modalities = ("image", "video")
    _can_record_outputs = {
        "hidden_states": ReQwen_1VisionBlock,
        "attentions": ReQwen_1VisionAttention,
    }
    ...
    def __init__(self, config, *inputs, **kwargs) -> None:
        ...
        self.patch_embed = ReQwen_1VisionPatchEmbed(
            config=config,
        )
        ...
        self.rotary_pos_emb = ReQwen_1VisionRotaryEmbedding(head_dim // 2)

        self.blocks = nn.ModuleList([ReQwen_1VisionBlock(config) for _ in range(config.depth)])
        self.merger = ReQwen_1VisionPatchMerger(
            config=config,
...
@auto_docstring
@dataclass
class ReQwen_1ModelOutputWithPast(BaseModelOutputWithPast):
    r"""
...
class ReQwen_1TextModel(ReQwen_1PreTrainedModel):
    ...
    def __init__(self, config: ReQwen_1TextConfig):
        ...
        self.layers = nn.ModuleList(
            [ReQwen_1DecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self.norm = ReQwen_1RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
@auto_docstring
class ReQwen_1Model(ReQwen_1PreTrainedModel):
    base_model_prefix = "model"
...
@auto_docstring
class ReQwen_1ForCausalLM(ReQwen_1PreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        ```python
        >>> from transformers import AutoTokenizer, ReQwen_1ForCausalLM

        >>> model = ReQwen_1ForCausalLM.from_pretrained("Qwen/ReQwen_1-8B")
        >>> tokenizer = AutoTokenizer.from_pretrained("Qwen/ReQwen_1-8B")
...
@auto_docstring
@dataclass
class ReQwen_1CausalLMOutputWithPast(CausalLMOutputWithPast):
    r"""
    rope_deltas (`torch.LongTensor` of shape `(batch_size, )`, *optional*):
        The rope index difference between sequence length and multimodal rope.
        The attribute is deprecated and will be removed in v5.20, use `model.base_model.rope_deltas` instead.
...
class ReQwen_1ForConditionalGeneration(ReQwen_1PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.model = ReQwen_1Model(config)
    ...
    def forward(
        ...
        logits_to_keep: int | torch.Tensor = 0,
        **kwargs: Unpack[TransformersKwargs],
    ) -> tuple | ReQwen_1CausalLMOutputWithPast:
        r"""
        labels (`torch.LongTensor` of shape `(batch_size, sequence_length)`, *optional*):
            Labels for computing the masked language modeling loss. Indices should either be in `[0, ...,
            config.vocab_size]` or -100 (see `input_ids` docstring). Tokens with indices set to `-100` are ignored
        ...
        ```python
        >>> from transformers import AutoProcessor, ReQwen_1ForConditionalGeneration

        >>> model = ReQwen_1ForConditionalGeneration.from_pretrained("Qwen/Qwen3-VL-8B-Instruct")
        ...
        return ReQwen_1CausalLMOutputWithPast(
            loss=loss,
            logits=logits,
            past_key_values=outputs.past_key_values,
            hidden_states=outputs.hidden_states,
    ...
    def _expand_inputs_for_generation(
        ...
        **model_kwargs,
        ...
        # Overwritten -- ReQwen_1 use timestamps and remove second_per_grid_ts
        ...
        def _expand_dict_for_generation_visual(dict_to_expand):
            ...
            # but ReQwen_1 append vision_start to each frame of each video, so we need to recover the real video_nums according to video_grid_thw
            if video_grid_thw is not None:
